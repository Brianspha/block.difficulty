...
contract Sablier is IERC1620, OwnableWithoutRenounce, PausableWithoutRenounce, Exponential, ReentrancyGuard {
     ...
     */
    function createStream(address recipient, address owner,uint256 deposit, address tokenAddress, uint256 startTime, uint256 stopTime)
        public
        whenNotPaused
        returns (uint256)
    {
        ...
        require(recipient != owner, "stream to the caller");
        ...
        require(IERC20(tokenAddress).transferFrom(owner, address(this), deposit), "token transfer failure");
        emit CreateStream(streamId, owner, recipient, deposit, tokenAddress, startTime, stopTime);
        return streamId;
    }
    ...
    function createCompoundingStream(
        ...
        uint256 streamId = createStream(recipient, msg.sender,deposit, tokenAddress, startTime, stopTime);
